...
UROUND = 2.0**-51  # deliberately 4x binary64 unit roundoff
...
def exact_to_float_tensor(expr):
    ...
    e0=np.nextafter(float(maxerr),np.inf) if maxerr else 0.0
    maxabs=float(np.max(np.abs(B)))
    Mbound=np.nextafter(2.0*(maxabs+1.0),np.inf)
    return B,deg,e0,Mbound,maxerr
...
class PolyState:
    ...
    def split(self,axis):
        ...
        # At most deg[axis] rounded averages contribute to any child coefficient.
        e=np.nextafter(self.err + self.deg[axis]*UROUND*self.Mbound,np.inf)
        return PolyState(self.name,L,self.sense,self.deg,e,self.Mbound), PolyState(self.name,R,self.sense,self.deg,e,self.Mbound)
    def prune(self):
        lo=float(np.min(self.B)); hi=float(np.max(self.B)); e=self.err
        if self.sense=='zero':
            if lo>e:return True,lo-e,'positive'
            if hi<-e:return True,-hi-e,'negative'
        elif self.sense=='le0':
            if lo>e:return True,lo-e,'positive'
        elif self.sense=='ge0':
            if hi<-e:return True,-hi-e,'negative'
...
def build_poly(name,expr,sense):
    t=time.time();B,deg,e0,Mbound,maxerr=exact_to_float_tensor(expr)
    return PolyState(name,B,sense,deg,e0,Mbound), {'seconds':time.time()-t,'degree':deg,'shape':list(B.shape),
            'max_initial_exact_to_float_error':float(maxerr),'roundoff_bound_initial':e0,'Mbound':Mbound}
...
def run_certificate(delta,h0,out_json,common=None):
    ...
    if common is None:
        ...
        # deep copy arrays because subdivision mutates only through newly allocated arrays, originals safe
    ...
    # order indices after states K,Cu,Cv,Nh,Cap
    order=[4,3,0,1,2]
    stack=[((0,0,0,0,0,0),states)]
    visited=0;prunes=Counter();minmargin={s.name:float('inf') for s in states};maxerr_seen={s.name:s.err for s in states}
    ...
    while stack:
        ...
        for idx in order:
            ...
            maxerr_seen[S[idx].name]=max(maxerr_seen[S[idx].name],S[idx].err)
            if yes:
                prunes[S[idx].name]+=1
                if margin<minmargin[S[idx].name]:minmargin[S[idx].name]=margin
                killed=True;break
        if killed:continue
    ...
    result={
      ...
      'min_certified_Bernstein_sign_margin':{k:(None if v==float('inf') else v) for k,v in minmargin.items()},
      'max_coefficient_error_bound_seen':maxerr_seen,'build':build,'subdivision_seconds':elapsed,
      'arithmetic_note':'Initial Bernstein coefficients are exact Fractions. Each is rounded once to binary64; the exact conversion error is computed. During de Casteljau subdivision, a scalar absolute-error envelope is enlarged by degree_axis * 2^-51 * Mbound per half-split, four times the standard binary64 unit roundoff. A sign is used only when every floating Bernstein coefficient is separated from zero by more than this rigorous envelope.',
...
if __name__=='__main__':
    common=build_common()
    for delta,h0,fn in [(Fraction(1,50),Fraction(24,25),'jnr_interval_delta_0p02.json'),
                        (Fraction(1,20),Fraction(9,10),'jnr_interval_delta_0p05.json')]:
        print('RUN',delta,h0,flush=True)
        R=run_certificate(delta,h0,fn,common)
        print(json.dumps({k:R[k] for k in ['status','delta_float','h0_float','rho_delta_lower_bound','epsilon_delta_lower_bound','visited_boxes','prune_counts','max_axis_depths','min_certified_Bernstein_sign_margin','max_coefficient_error_bound_seen','subdivision_seconds']},indent=2),flush=True)
